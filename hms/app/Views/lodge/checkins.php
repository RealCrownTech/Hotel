								...
								<tbody>
									<?php if(!empty($lodges)): ?>
										<?php $i = 1; foreach ($lodges as $row): ?>
											<tr>
												<input type="hidden" class="lodge_id" value="<?= $row['lodge_no']; ?>">
												...
												<?php if(in_array('editLodge', $user_permission) || in_array('viewLodge', $user_permission) || in_array('deleteLodge', $user_permission)): ?>
													<td>
														...
														<?php if(in_array('editLodge', $user_permission)): ?>
															<a href="<?= base_url() ?>/editLodge/<?= $row['lodge_no']; ?>" class="btn btn-sm btn-secondary"><i data-feather='edit'></i></a>
															<a href="<?= base_url() ?>/swap/<?= $row['lodge_no']; ?>" class="btn btn-sm btn-info"><i data-feather='repeat'></i></a>
														<?php endif ?>
	...
	<script type="text/javascript">
		jquery_3_2_1(document).ready(function () {
			$(document).on('click', '.check_out', function(){
				var lodge_no = $(this).closest('tr').find('.lodge_id').val();
				$('#lodge_id_modal').val(lodge_no);
			});
		});
	</script>
