								...
								<tbody>
									<?php if(!empty($orders)): ?>
										<?php $i = 1; foreach ($orders as $row): ?>
											<tr>
												<input type="hidden" class="stock_order_id" name="" value="<?= $row['order_id']; ?>">
												<td><?= $i ?></td>
												...
												<?php if(in_array('editStockOrder', $user_permission) || in_array('viewStockOrder', $user_permission) || in_array('deleteStockOrder', $user_permission)): ?>
													<td>
														<?php if(in_array('viewStockOrder', $user_permission)): ?>
															<a href="<?= base_url() ?>/viewStockOrder/<?= $row['order_id'] ?>" class="btn btn-sm btn-warning"><i data-feather="eye"></i></a>
														<?php endif ?>
														<?php if(in_array('editStockOrder', $user_permission)): ?>
															<a href="<?= base_url() ?>/editStockOrder/<?= $row['order_id'] ?>" class="btn btn-sm btn-info"><i data-feather="edit"></i></a>
														<?php endif ?>
														<?php if(in_array('deleteStockOrder', $user_permission)): ?>
	...
	<script type="text/javascript">
		jquery_3_2_1(document).ready(function () {
			$(document).on('click', '.delete', function(){
				var delete_id = $(this).closest('tr').find('.stock_order_id').val();
				$('#deleteModal').modal('show');
				$('#deleteForm').attr('action', '<?= base_url() ?>/deletestockorder/'+delete_id);
			});
		});
	</script>
