													...
													<div class="row row_count" id="row_1">
														<div class="col-12 col-md-3">
															<div class="mb-3 mb-xl-0">
																...
																<select class="form-control select2" data-toggle="select2" id="item_1" name="item[]" onchange="getStockData(1)">
																	<option value="">--Search--</option>
																	<?php if(!empty($stock)): ?>
																	<?php foreach($stock as $row): ?>
														            <option value="<?= $row['stock_id'] ?>"><?= $row['stock_name'] ?></option>
														        	<?php endforeach ?>
														        	<?php endif ?>
														        </select>
															</div>
														...
														<div class="col-12 col-md-2">
															<div class="mb-3 mb-xl-0">
																<label class="form-label">Stock<span class="font-13 text-danger">*</span></label>
																<input class="form-control" type="text" name="stock[]" id="stock_1" readonly/>
															</div>
														</div>
														<div class="col-12 col-md-2">
															<div class="mb-3 mb-xl-0">
	...
	<script type="text/javascript">
		jquery_3_2_1(document).ready(function() {
			...
			jquery_3_2_1("#add_new_row").unbind('click').bind('click', function() {
			    ...
			    $.ajax({
			        method: 'POST',
					url: 'ajax.stock/fetchAllStockData',
					data: {},
			        success:function(response) {	
			        	var html = '<div class="row row_count mb-3" id="row_'+row_id+'">'+
				        	'<div class="col-12 col-md-3">'+
				        		'<div class="mb-3 mb-xl-0">'+
				        			'<select class="form-control form_select select2" data-toggle="select2" id="item_'+row_id+'" name="item[]" onchange="getStockData('+row_id+')">'+
				        				'<option value="">--Search--</option>';
				        				$.each(response.stock_data, function(key, stockvalue) {
				                        	html += '<option value="'+stockvalue.stock_id+'">'+stockvalue.stock_name+'</option>';             
				                        });
				        			html += '</select>'+
				        		'</div>'+
				        	'</div>'+
				        	'<div class="col-12 col-md-2">'+
								'<div class="mb-3 mb-xl-0">'+
									'<input class="form-control" type="text" name="stock[]" id="stock_'+row_id+'" readonly/>'+
								'</div>'+
							'</div>'+
				        	'<div class="col-12 col-md-2">'+
								'<div class="mb-3 mb-xl-0">'+
    		...
    		$(document).on('change', '#choose_room', function(){
				...
				$.ajax({
					method: 'POST',
					url: 'ajax.stock/fetchGuestRoomAndData',
		...
		function getTotal(row = null) {
		    if(row) {
		    	var stock_value = $('#stock_'+row).val();
				var quantity = $('#qty_'+row).val();
				if (parseFloat(quantity) > parseFloat(stock_value)) {
					parseFloat($('#qty_'+row).val(stock_value));
				}else{
					parseFloat($('#qty_'+row).val(quantity));
				}
		...
		function getStockData(row_id)
		{
		    var stock_id = $("#item_"+row_id).val();    
		    if(stock_id == "") {
		      $("#rate_"+row_id).val("");
		      $("#qty_"+row_id).val(""); 			  
			  $("#stock_"+row_id).val("");
		      ...
		      $.ajax({
		        ...
		        success:function(response) {
		        	$.each(response, function (key, stockvalue) {
						$("#rate_"+row_id).val(stockvalue['selling_price']);
						$("#stock_"+row_id).val(stockvalue['stock_qty']);
					});		          
		          subAmount();
		        } // /success
		      });
