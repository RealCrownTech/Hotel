								...
								<tbody>
									<?php if(!empty($orders)): ?>
										<?php $i = 1; foreach ($orders as $row): ?>
											<tr>
												<input type="hidden" class="amenities_order_id" name="" value="<?= $row['order_id']; ?>">
												<td><?= $i ?></td>
												...
												<?php if(in_array('editAmenitiesOrder', $user_permission) || in_array('viewAmenitiesOrder', $user_permission) || in_array('deleteAmenitiesOrder', $user_permission)): ?>
													<td>
														<?php if(in_array('viewAmenitiesOrder', $user_permission)): ?>
															<a href="<?= base_url() ?>/viewAmenitiesOrder/<?= $row['order_id'] ?>" class="btn btn-sm btn-warning"><i data-feather="eye"></i></a>
														<?php endif ?>
														<?php if(in_array('editAmenitiesOrder', $user_permission)): ?>
															<a href="<?= base_url() ?>/editAmenitiesOrder/<?= $row['order_id'] ?>" class="btn btn-sm btn-info"><i data-feather="edit"></i></a>
														<?php endif ?>	
														<?php if(in_array('deleteAmenitiesOrder', $user_permission)): ?>
	...
	<script type="text/javascript">
		jquery_3_2_1(document).ready(function () {
			$(document).on('click', '.delete', function(){
				var delete_id = $(this).closest('tr').find('.amenities_order_id').val();
				$('#deleteModal').modal('show');
				$('#deleteForm').attr('action', '<?= base_url() ?>/deleteamenitiesorder/'+delete_id);
			});
		});
	</script>
