				...
				<div class="col-12">
					<div class="card">
						<div class="card-body">
							...
							<hr class="my-4">
							<?= form_open('/createAmenitiesOrder') ?>
								<div class="row">
									<div class="col-md-12">
										<div class="card">
											<div class="card-body">
													...
													<div class="row row_count" id="row_1">
														<div class="col-12 col-md-3">
															<div class="mb-3 mb-xl-0">
																...
																<select class="form-control select2" data-toggle="select2" id="item_1" name="item[]" onchange="getAmenitiesData(1)">
																	<option value="">--Search--</option>
																	<?php if(!empty($amenities)): ?>
																	<?php foreach($amenities as $row): ?>
														            <option value="<?= $row['amenities_id'] ?>"><?= $row['amenities_name'] ?></option>
														        	<?php endforeach ?>
														        	<?php endif ?>
														        </select>
															</div>
	...
	<script type="text/javascript">
		jquery_3_2_1(document).ready(function() {
			...
			jquery_3_2_1("#add_new_row").unbind('click').bind('click', function() {
			    ...
			    $.ajax({
			        method: 'POST',
					url: 'ajax.amenities/fetchAllAmenitiesData',
					data: {},
			        success:function(response) {	
			        	var html = '<div class="row row_count mb-3" id="row_'+row_id+'">'+
				        	'<div class="col-12 col-md-3">'+
				        		'<div class="mb-3 mb-xl-0">'+
				        			'<select class="form-control form_select select2" data-toggle="select2" id="item_'+row_id+'" name="item[]" onchange="getAmenitiesData('+row_id+')">'+
				        				'<option value="">--Search--</option>';
				        				$.each(response.amenities_data, function(key, amenitiesvalue) {
				                        	html += '<option value="'+amenitiesvalue.amenities_id+'">'+amenitiesvalue.amenities_name+'</option>';             
				                        });
				        			html += '</select>'+
				        		'</div>'+
				        	'</div>'+
    		...
    		$(document).on('change', '#choose_room', function(){
				...
				$.ajax({
					method: 'POST',
					url: 'ajax.amenities/fetchGuestRoomAndData',
		...
		function getAmenitiesData(row_id)
		{
		    var amenities_id = $("#item_"+row_id).val();    
		    if(amenities_id == "") {
		      $("#rate_"+row_id).val("");
		      $("#qty_"+row_id).val(""); 			  
			  $("#stock_"+row_id).val("");
		      ...
		      $.ajax({
		        ...
		        success:function(response) {
		        	$.each(response, function (key, amenitiesvalue) {
						$("#rate_"+row_id).val(amenitiesvalue['amenities_price']);
					});		          
		          subAmount();
		        } // /success
		      });
